import { loadPlayerProgress } from '$lib/server/player-progress';
...
export const load: PageServerLoad = async ({ locals, params }) => {
	...
	try {
		...
		const weaponDistances = access.caps.has('integrity.view')
			...
			: null;
		const playerProgress = access.caps.has('integrity.view')
			? await loadPlayerProgress(env, server.id, params.steamId)
			: null;
		return {
			playerProgress,
			dossier: d,
			career,
			multiServer: visible.length > 1,
			integrity,
			weaponDistances
		};
	} catch (err) {
