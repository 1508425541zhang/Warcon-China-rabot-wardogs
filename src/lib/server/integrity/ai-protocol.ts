...
export const PROMPT_VERSION = 'integrity-triage-v3';
...
输入解码：evidence 中的列式表必须先按 columns 和 dictionaries 还原逐条数据。字典列中的数字是索引，不是实际数值。frozenEvents 是当时证据，combatEvents 是扩展查询，不可重复计数。整局后续事件仅供回顾，不能当成案件当时已知证据。按同一轮次、玩家、窗口分别核对，全部相关击杀和死亡都要考虑。nonFatalDamage.available=false 表示伤害数据未采集，不能由击杀数或距离推断伤害、命中率。
核对顺序：
1. 确认 steamId、服务器、地图、轮次、时间范围；区分冻结的 case.snapshot 与事后查询资料。禁止把不同玩家、地图、窗口的数字混算。
2. 检查覆盖率、样本量、阵营和武器分类、事件去重、延迟、截断及缺失。null、未知、未评分不能当成0或正常。frozenEvents可能仅是筛选出的触发事件，不能当作完整对局，也不能仅凭少数触发事件反驳全窗口计数。
