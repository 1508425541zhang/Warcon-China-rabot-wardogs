...
3. Worker 依据游戏时钟维护滚动 180 秒窗口。KPM = 有效击杀数 ÷ 3。换图、实例变化或明显时钟回退会清空窗口；异常发现之间须相隔至少 180 秒，避免同一连续事件被重复记分。
4. 分数由当前 KPM 档、独立受害者、独立异常窗口、独立举报人等分项形成，每个分项保留解释。KPM 档只取最高命中档，不累加。KD 仅展示。Steam 先验和举报不会单独构成当前行为异常。
5. 达到配置的案件阈值时保存 `CASE-…`、命中事件、规则版本与分项。证据完整度按现有数据标为 B/C/D；A 等级尚需连续 Feed 验证。案件供人工审核，不等同作弊结论。
...
爆头率、穿透率、短时爆发、Steam 处罚和公开游戏时间虽然有参数位置，目前缺少可信输入，**不会参与实时评分**。无 Kill Feed 或数据截断时，不应把空值解释为“0 KPM”。
