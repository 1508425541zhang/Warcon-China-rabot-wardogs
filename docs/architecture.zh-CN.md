...
2. 武器分类只把确认的步兵武器列为 `INFANTRY`。`UNKNOWN`、载具、迫击炮、固定武器、环境、自杀、队友击杀与阵营不明事件不进入纯步兵 KPM。
   Kill Feed 原始事件不含双方阵营。接收时只读取十秒内、地图状态与玩家列表时间相符的实时玩家列表，保存列表观察时间；Integrity 作业等下一次玩家列表观察，双方阵营在前后两次观察中一致时才纳入 KPM。玩家会话阵营有心跳延迟，不再用于击杀阵营。若玩家离线、换边、轮询超时或地图不符，事件保留，但 KPM 标为不可用而不是 0。当前协议仍无法证明两次观察之间瞬时换边的精确时刻。
